...
// Define the native command
var nativeCmd = &cobra.Command{
	Use:     "native",
	Short:   "Run the native Go service",
	Example: "go run main.go native --watched-dir=./definitions",
...
func init() {
	...
	// Add nativeCmd as a subcommand to rootCmd
	rootCmd.AddCommand(nativeCmd)

	// Define the --watched-dir flag for nativeCmd
	nativeCmd.Flags().StringVar(&watchedDir, "watched-dir", "",
		"The directory to watch for changes.")

	// Mark the --watched-dir flag as required
	nativeCmd.MarkFlagRequired("watched-dir") // nolint:errcheck
}
